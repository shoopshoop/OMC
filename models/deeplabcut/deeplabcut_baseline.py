...
'''
Project creation guidlines found at
https://github.com/DeepLabCut/DeepLabCut/wiki/Using-labeled-data-in-DeepLabCut-that-was-annotated-elsewhere
'''

def create_new_project():
  return deeplabcut.create_new_project('omc_deeplabcut', 'anthony',['./dummy_video.avi'], copy_videos=True, multianimal=False)

def generate_csv(ann_file, img_dir):
...
if __name__ == '__main__':
  # create new project
  # make sure to include a './dummy_video.avi' file
  # and edit the config.yaml file to include correct
  # bodyparts and skeleton information
  '''
  config = create_new_project()
  '''
  # path to config file if project is already created
  config = '/home/anthony/UMN_3-3-2022/CSCI_5561/final_project/final/src/omc_deeplabcut-anthony-2022-04-22/config.yaml'

  # convert labeled data to appropriate csv format
  '''
  data_root = '../../data/'
  ann_file = data_root + 'train_annotation_cropped.json'
  img_dir = data_root + 'train_cropped/'
  generate_csv(ann_file, img_dir)
  '''
  # make sure to move the csv file to labeled-data/dummy_video/ before the following step
  ...
  '''
  # train model
  # to start training from an existing checkpoint, change the init_weights field in dlc_models/.../train/pose_cfg.yaml
  '''
  ...
  # make sure to active conda environment: > conda activate dlc-live
  om_train = OpenMonkey('../../data/res/test_prediction.json', '../../data/test/')
  model_path = os.path.abspath('.') + '/omc_deeplabcut-anthony-2022-04-22/exported-models/DLC_omc_deeplabcut_resnet_50_iteration-0_shuffle-1/'
  ...
  for i in range(len(om_train.imgs)):
    ...
    x1, y1 = om_train.bbox[i][0], om_train.bbox[i][1]
    x2, y2 = x1 + om_train.bbox[i][2], y1 + om_train.bbox[i][3]
    img = img[y1:y2, x1:x2,:] # crop
    om_train.landmarks[i] = dlc_live.get_pose(img)[:, :2].flatten().tolist()
  om_train.write_landmarks_to_file(relative=True) # write global landmarks to file
