...
# size of image fed into model
scaled_size = (256,256)

# filepaths
train_json_file = '../../data/train_annotation.json'
train_img_dir = '../../data/train/'
val_json_file = '../../data/val_annotation.json'
...
def main(config):
    batch_size = 32
    dataset_train = MonkeyDataset(train_img_dir, train_json_file)
    sampler_train = torch.utils.data.DistributedSampler(dataset_train, shuffle=True)
    dataset_val = MonkeyDataset(val_img_dir, val_json_file)
    sampler_val = torch.utils.data.distributed.DistributedSampler(dataset_val, shuffle=False)
    data_loader_train = DataLoader(dataset_train, batch_size=batch_size, sampler=sampler_train)
    ...
    if config.MODEL.RESUME:
        min_loss = load_checkpoint(config, model_without_ddp, optimizer, lr_scheduler, logger)
        loss = validate(config, data_loader_val, model)
        logger.info(f"Loss of the network on the {len(dataset_val)} validation images: {loss:.1f}")
        if config.EVAL_MODE:
            return
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
            save_checkpoint(config, epoch, model_without_ddp, min_loss, optimizer, lr_scheduler, logger)

        loss = validate(config, data_loader_val, model)
        logger.info(f"Loss of the network on the {len(dataset_val)} validation images: {loss:.1f}")
        min_loss = min(min_loss, loss)
        logger.info(f'Min loss: {min_loss:.1f}')
